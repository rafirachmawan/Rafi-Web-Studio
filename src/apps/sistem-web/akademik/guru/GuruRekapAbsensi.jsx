...
export default function GuruRekapAbsensi() {
  ...
  const records = absensiRecords.filter((a) => siswaIds.includes(a.siswaId));
            ...
            <tbody>
              {perSiswa.map((s, i) => (
                <tr key={s.id} className="border-b border-white/[0.03] hover:bg-white/[0.03]">
                  ...
                  <td className="px-4 py-2 text-center text-white font-bold">
                    {s.total ? Math.round((s.hadir / s.total) * 100) : 0}%
                  </td>
                </tr>
              ))}
            </tbody>

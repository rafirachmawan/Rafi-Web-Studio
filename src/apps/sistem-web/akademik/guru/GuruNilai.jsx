...
import { DEMO_USERS, KELAS_LIST, MAPEL_LIST, getSiswaByKelas, NILAI_LIST } from "../data/mockData";
import { CheckCircle2 } from "lucide-react";

export default function GuruNilai() {
  const user = DEMO_USERS.guru;
  const guruMapel = MAPEL_LIST.filter((m) => m.guruId === user.id);
  ...
  const getNilai = (siswaId) => {
    ...
    const found = NILAI_LIST.find((n) => n.siswaId === siswaId && n.mapelId === selectedMapel);
    return found || { tugas: "", uts: "", uas: "" };
  };

  const updateNilai = (siswaId, field, value) => {
    const v = Math.min(100, Math.max(0, parseInt(value) || 0));
    setNilaiState((prev) => ({ ...prev, [siswaId]: { ...getNilai(siswaId), [field]: v } }));
    setSaved(false);
  };
  ...
  return (
    <>
      <TopBar title="Input Nilai" subtitle={user.mapel} user={user} />
      <div className="p-6 space-y-4">
        <div className="flex flex-wrap items-center gap-2">
          {KELAS_LIST.map((k) => (
            <button key={k.id} onClick={() => { setSelectedKelas(k.id); setSaved(false); }}
          ...
          {guruMapel.map((m) => (
            <button key={m.id} onClick={() => { setSelectedMapel(m.id); setSaved(false); }}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${selectedMapel === m.id ? "bg-amber-500/15 border-amber-500/30 text-amber-400" : "bg-white/[0.02] border-white/[0.06] text-zinc-400 hover:bg-white/[0.05]"}`}>
              {m.nama}
            </button>
          ))}
        </div>
              ...
              {siswaList.map((s, i) => {
                ...
                return (
                  <tr key={s.id} className="border-b border-white/[0.03]">
                    ...
                    <td className="px-4 py-2.5 text-zinc-300"><div className="flex items-center gap-2"><img src={s.avatar} alt="" className="w-6 h-6 rounded-md object-cover" />{s.nama}</div></td>
                    {["tugas", "uts", "uas"].map((f) => (
                      <td key={f} className="px-2 py-2.5 text-center">
                        <input type="number" min="0" max="100" value={n[f]} onChange={(e) => updateNilai(s.id, f, e.target.value)}
                    ...
                    <td className="px-4 py-2.5 text-center">
                      <span className={`text-xs font-bold ${typeof avg === "number" && avg >= 75 ? "text-emerald-400" : typeof avg === "number" ? "text-amber-400" : "text-zinc-600"}`}>{avg}</span>
                    </td>
                  </tr>
                );
              })}
        ...
        <div className="flex justify-end">
          {saved ? (
            ...
            <button onClick={handleSave} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white text-sm font-semibold hover:opacity-90 transition-opacity">
              💾 Simpan Nilai
            </button>
          )}
        </div>

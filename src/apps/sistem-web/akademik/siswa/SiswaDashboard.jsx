...
export default function SiswaDashboard() {
  const { mapelList, jadwal, getNilaiSiswa, getAbsensiSiswa, demoUsers } = useAkademik();
  ...
  const avgNilai = nilai.length
    ? Math.round(
        nilai.reduce(
          (sum, n) =>
            sum +
            ((parseInt(n.tugas) || 0) + (parseInt(n.uts) || 0) + (parseInt(n.uas) || 0)) / 3,
          0
        ) / nilai.length
      )
    : 0;
  ...
  return (
    <>
      <TopBar title="Dashboard" subtitle={`Selamat datang, ${user.nama || "Siswa"}`} user={user} />
                ...
                return (
                  <div key={i} className="flex items-center gap-4 p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.05] transition-colors">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-[10px] text-zinc-500">{user.kelas || "Belum ada kelas"}</p>
                    </div>

...
export default function SiswaNilai() {
  const { mapelList, getNilaiSiswa, demoUsers } = useAkademik();
  ...
  const rows = nilai.map((n) => {
    ...
    const avg = Math.round(((parseInt(n.tugas) || 0) + (parseInt(n.uts) || 0) + (parseInt(n.uas) || 0)) / 3);
  ...
  return (
    <>
      <TopBar title="Nilai Saya" subtitle={`${user.kelas || "Belum ada kelas"} — Semester Genap`} user={user} />

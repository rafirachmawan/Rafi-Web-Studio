...
export default function AdminAbsensi() {
  ...
  const kelasRecords = ABSENSI_RECORDS.filter((a) => siswaIds.includes(a.siswaId));
  ...
  const total = kelasRecords.length;
  ...
  return (
    <>
      <TopBar title="Rekap Absensi" subtitle="Semua kelas" user={DEMO_USERS.admin} />
      <div className="p-6 space-y-4">
        <div className="flex items-center gap-2 flex-wrap">
          {KELAS_LIST.map((k) => (
            <button key={k.id} onClick={() => setSelectedKelas(k.id)}
              className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${selectedKelas === k.id ? "bg-indigo-500/15 border-indigo-500/30 text-indigo-400" : "bg-white/[0.02] border-white/[0.06] text-zinc-400 hover:bg-white/[0.05]"}`}>
              {k.nama}
            </button>
          ))}
        </div>
          ...
          return (
            <div key={date} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
              <div className="px-4 py-3 border-b border-white/[0.06] bg-white/[0.02]">
                <h4 className="text-xs font-semibold text-white">{new Date(date).toLocaleDateString("id-ID", { weekday: "long", year: "numeric", month: "long", day: "numeric" })}</h4>
              </div>
              <table className="w-full text-xs">
                ...
                <tbody>{dayRecords.map((r, i) => {
                  const siswa = SISWA_LIST.find((s) => s.id === r.siswaId);
                  return (
                    <tr key={i} className="border-b border-white/[0.03] hover:bg-white/[0.03]">
                      <td className="px-4 py-2 text-zinc-300"><div className="flex items-center gap-2"><img src={siswa?.avatar} alt="" className="w-6 h-6 rounded-md object-cover" />{siswa?.nama}</div></td>
                      <td className="px-4 py-2"><span className={`px-2 py-0.5 rounded-md text-[10px] font-semibold capitalize ${sc[r.status]}`}>{r.status}</span></td>
